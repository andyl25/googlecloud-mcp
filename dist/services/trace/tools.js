 ...
 */
export async function registerTraceTools(server) {
    // Tool to get a trace by ID
    server.tool('get-trace', {
        ...
        try {
            ...
            if (typeof traceId === 'string' && !traceId.match(/^[a-f0-9]+$/i)) {
                throw new GcpMcpError('Invalid trace ID format. Trace ID should be a hexadecimal string.', 'INVALID_ARGUMENT', 400);
            }
            // Initialize Google Auth client
            const auth = await initGoogleAuth(true);
            if (!auth) {
                throw new GcpMcpError('Google Cloud authentication not available. Please configure authentication to access trace data.', 'UNAUTHENTICATED', 401);
            }
            ...
            if (!response.ok) {
                ...
                throw new GcpMcpError(`Failed to fetch trace: ${errorText}`, 'FAILED_PRECONDITION', response.status);
            }
            ...
            try {
                ...
                const hierarchicalTrace = buildTraceHierarchy(actualProjectId.toString(), traceId.toString(), traceData.spans);
            ...
            throw new GcpMcpError(`Failed to fetch trace: ${error.message}`, error.code || 'UNKNOWN', error.statusCode || 500);
        }
    });
    // Tool to list recent traces
    server.tool('list-traces', {
        projectId: z.string().optional().describe('Optional Google Cloud project ID'),
        ...
        try {
            ...
            if (startTime) {
                ...
                if (startTimeStr.match(/^\d+[hmd]$/)) {
                    ...
                    try {
                        ...
                        throw new GcpMcpError(`Invalid start time format: "${startTimeStr}". Use ISO format or relative time (e.g., "1h", "2d").`, 'INVALID_ARGUMENT', 400);
                    }
                }
            }
            else {
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    throw new GcpMcpError(`Failed to list traces: ${errorText}`, 'FAILED_PRECONDITION', response.status);
                }
                ...
                throw new GcpMcpError(`Failed to fetch traces: ${fetchError.message}`, 'INTERNAL', 500);
            }
            ...
            throw new GcpMcpError(`Failed to list traces: ${error.message}`, error.code || 'UNKNOWN', error.statusCode || 500);
        }
    });
    // Tool to find traces associated with logs
    server.tool('find-traces-from-logs', {
        projectId: z.string().optional().describe('Optional Google Cloud project ID'),
        ...
        try {
            ...
            throw new GcpMcpError(`Failed to find traces from logs: ${error.message}`, error.code || 'UNKNOWN', error.statusCode || 500);
        }
    });
    // Tool to analyze a trace using natural language
    server.tool('natural-language-trace-query', {
        ...
        try {
            ...
            if (traceIdMatch && traceIdMatch[1]) {
                ...
                const response = await fetch(`https://cloudtrace.googleapis.com/v1/projects/${actualProjectId}/traces/${traceId}`, {
                    headers: {
                        'Authorization': `Bearer ${token.token}`,
            ...
            throw new GcpMcpError(`Failed to process natural language query: ${error.message}`, error.code || 'UNKNOWN', error.statusCode || 500);
        }
    });
}
/**
 ...
 */
function formatTracesResponse(tracesData, projectId, startTime, endTime, filter) {
    // Format the traces for display
